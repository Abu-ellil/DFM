...
// Configuration
const GITHUB_TOKEN = process.env.GITHUB_TOKEN || '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';
...
function getReleaseNotes() {
  ...
  const versionSection = changelog.match(new RegExp(`\\[${versionRegex}\\]([\\s\\S]*?)(?=\\n## \\[|\\n---|$)`));
...
async function main() {
  try {
    ...
    if (assets.length === 0) {
      ...
      console.warn('Please build the project first using: npm run build && npm run build:win && npm run build:mac && npm run build:linux');
    } else {
