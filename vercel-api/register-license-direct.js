...
async function main() {
  console.log('📝 Registering license key directly to database...\n')
  ...
  try {
    ...
    const existing = await sql`
      SELECT * FROM license_keys WHERE license_key = ${licenseKey.toUpperCase()}
    `

    if (existing.length > 0) {
      console.log('⚠️  License key already exists, updating...')
      await sql`
    ...
    console.log('')
    console.log('If sync still fails, make sure your app is using this license key.')
    console.log('You can activate it in the app by entering:', licenseKey)
  } catch (error) {
