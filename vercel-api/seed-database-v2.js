/**
 * Database Seed Script for Dates Factory Manager (V2)
 *
...
async function seedCustomers(sql) {
  ...
  for (const customer of sampleCustomers) {
    try {
      ...
      console.log(`  Skipped duplicate: ${customer.name}`)
    }
  }
  return ids
}
...
async function seedDateTypes(sql) {
  ...
  for (const dateType of sampleDateTypes) {
    try {
      ...
      // Skip duplicate entries
      console.log(`  Skipped duplicate: ${dateType.name}`)
    }
  }
  return ids
}
...
async function seedCrateTypes(sql) {
  ...
  for (const crateType of sampleCrateTypes) {
    try {
      ...
      // Skip duplicate entries
      console.log(`  Skipped duplicate: ${crateType.name}`)
    }
  }
  return ids
}
...
async function seedSupervisors(sql) {
  ...
  for (const supervisor of sampleSupervisors) {
    try {
      ...
      // Skip duplicate entries
      console.log(`  Skipped duplicate: ${supervisor.name}`)
    }
  }
  return ids
}
...
async function seedAuthUsers(sql) {
  ...
  for (const user of users) {
    try {
      ...
      // Skip duplicate entries
      console.log(`  Skipped duplicate user: ${user.phone}`)
    }
  }
}
